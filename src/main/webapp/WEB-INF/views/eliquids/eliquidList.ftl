...
<div id="goodsBlock">
    <p><a href="/eliquid/${goods.id}"><img width="100%" height="100%" src="/resources/image/${goods.image}"></a></p>
    <p>${goods.name}</p>
...
<#if pages??>
    <#list pages as page>
        <#if order??>
            <#if direction??>
                <span><a href="/eliquid/all?page=${page}&order=${order}&direction=${direction}">${page} ;</a></span>
            </#if>
        </#if><#else><span><a href="/eliquid/all?page=${page}">${page} ;</a></span>
    </#list>
</#if>
<br>
